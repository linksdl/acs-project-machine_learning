...
class Agent:
    ...
    def update_value_Q(self, s, a, r, s_next, done):
        ...
        if done:
            ...
            target_Q = r + self.gamma * np.max(self.Q[s_next, :])
        # update the Q-table， Bellman equation
        self.Q[s, a] += self.learning_rate * (target_Q - predict_Q)
    ...
    def choose_action(self, s):
        if np.random.rand() < self.epsilon:
            ...
            action_list = np.where(self.Q[s, :] == Q_max)[0]
            action = np.random.choice(action_list)

        return action

    def updateEpsilon(self, episode):
        self.epsilon = self.epsilon / (1 + 0.1 * episode)
...
if __name__ == '__main__':
    ...
    fig1 = plt.figure(1)
    plt.title('The q_Sinit_progress')
    plt.ion()
    ...
    # 2) plot the evolution of the number of steps per successful episode throughout training.
    # A successful episode is an episode where the agent reached the goal
    # (i.e. not any terminal state)
    fig2 = plt.figure(2)
    ...
    # 3) plot the evolution of the total collected rewards per episode throughout training.
    # you can use the running_mean function to smooth the plot
